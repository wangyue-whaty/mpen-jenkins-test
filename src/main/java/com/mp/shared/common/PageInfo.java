...
public class PageInfo implements Serializable, Comparable<PageInfo>, Comparator<PageInfo>, IsValid {
    ...
    public MpCode.Point crtc(MpCode mpCode, float prx, float pry) {
        ...
        final MpCode.Point point = new MpCode.Point();
        point.x = (int) ((prx + codeRealPixelX) / max(width, 1) * 1000);
        point.y = (int) ((pry + codeRealPixelY) / max(height, 1) * 1000);
        point.pageNum = this.pageNum;
        return point;
    }
